...
// --- Three.js Canvas Component (reusable background) ---
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        tempVelocity.set(velocities[ix], velocities[iy], velocities[iz]);

        const dist = tempCurrent.distanceTo(mouseWorld);
        if (dist < 1.5) {
          const force = (1.5 - dist) * 0.01;
          tempDirection.subVectors(tempCurrent, mouseWorld).normalize();
          tempVelocity.addScaledVector(tempDirection, force);
        }

        // Return to original
        tempVelocity.addScaledVector(
          tempDirection.subVectors(tempOriginal, tempCurrent),
          0.001
        );
        ...
        positions[ix] += tempVelocity.x;
        positions[iy] += tempVelocity.y;
        positions[iz] += tempVelocity.z;

        velocities[ix] = tempVelocity.x;
        velocities[iy] = tempVelocity.y;
        velocities[iz] = tempVelocity.z;
      }
